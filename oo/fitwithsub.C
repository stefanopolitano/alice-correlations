         ...
         double absDeltaPhiMax = 1.3)
{
    ...
    for (uint imult = 0; imult < Ncent; ++imult) // Loop over centrality
    {
        ...
        for (int imass = 0; imass < int(Nmass); ++imass) // Loop over mass bins
        {
            ...
            setTH1Style(phf1, "#Delta#varphi (rad)", Form("dN/d#Delta#varphi (|#Delta#eta| > %d)", int(absDeltaEtaMin)), kBlack);
            ...
            std::cout << "Mass bin " << imass << ": " << pmassBins[imass] << " - " << pmassBins[imass + 1] << " GeV/c^2, center = " << masscenter << " GeV/c^2" << std::endl;
            ...
            for (int i = 0; i < fit->GetNpar() + 1 + subtraction; i++)
            {
                ...
                else if (i == fit->GetNpar() + 1 && imult > 0)
                { // LM subtraction
...
void fitwithsub(bool subtraction = true, const char *inFilePath = "./lambdas_full", const char *outFileName = "./massandv2lambda", double absDeltaEtaMin = 1.2, double absDeltaEtaMax = 1.8, double absDeltaPhiMax = 1.3)
{
    ...
    // Load all files called dphi*.root from the input directory
    std::vector<std::string> files = GetAllRootFiles(inFilePath, true);
    std::sort(files.begin(), files.end());
    std::cout << "Found " << files.size() << " files to process." << std::endl;

    // Prepare output file name safely
    std::string outFileNameStr(outFileName);
    if (subtraction)
    {
        outFileNameStr += "Sub.root";
        std::cout << "Subtraction mode enabled. Output file will be: " << outFileNameStr << std::endl;
    }
    else
    {
        outFileNameStr += ".root";
        std::cout << "Subtraction mode disabled. Output file will be: " << outFileNameStr << std::endl;
    }

    // Check if output file already exists and remove if necessary
    if (TFile::Open(outFileNameStr.c_str()))
    {
        std::cout << "WARNING: Output file already exists: " << outFileNameStr << " - it will be overwritten!" << std::endl;
        TFile::Open(outFileNameStr.c_str())->Close(); // Close the file to avoid issues
        if (remove(outFileNameStr.c_str()) != 0)
            std::cerr << "ERROR: Failed to remove existing output file: " << outFileNameStr << std::endl;
        else
            std::cout << "Removed existing output file: " << outFileNameStr << std::endl;
    }
    else
    {
        std::cout << "Output file does not exist. Proceeding to create: " << outFileNameStr << std::endl;
    }
    ...
    for (int iFile = 0; iFile < files.size(); ++iFile) // Uncomment the condition to process all files: for (int iFile = 0; iFile < (int)
    {
        ...
        fit(f.c_str(), iFile, outFileNameStr.c_str(), subtraction, absDeltaEtaMin, absDeltaEtaMax, absDeltaPhiMax);
    }
